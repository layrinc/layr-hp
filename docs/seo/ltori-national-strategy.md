...
| FAQ・内部リンク | 複数勤務地の運用、地域外の候補者への案内を追加。FAQの表示と構造化データは同じ情報源。パンくず・都道府県からの子地域一覧で接続（末尾の他地域リンクは2026-09-18の追加指示で削除） |
...
地域LPの「ほかの対応地域」と直下の県・全国一覧リンクを削除。全国索引、都道府県から子地域へのリンク、パンくず、サイトマップは維持。`/tools/ltori-seo/` で1,965地域・標準9,825キーワード候補・検索実績・改善タスクを管理する。保存・実績の仕様は [管理ツールの設計と運用](ltori-keyword-manager.md) を参照。
