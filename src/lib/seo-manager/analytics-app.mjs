import {isServerReport,serverReportLabel,regionalServerStatus} from './regional-server-analytics.mjs';
...
export function mountAnalytics({catalog,getState,getServer=()=>({loading:false,data:null,error:''}),save,download}) {
  ...
  let active='', token='', expires=0, busy=false, pending=null, identityLoading, selectedByUser=false, autoDefaultUsed=false;
  ...
  const label = r=>isServerReport(r)?serverReportLabel(r):`${r.start}〜${r.end} / ${r.origin==='google'?'Google API':'CSV'} / GA4 ${r.property} / ${new Date(r.importedAt).toLocaleString('ja-JP')}`;
  const allReports = ()=>[...(getServer().data?.reports||[]),...(getState().analyticsReports||[])];
  const report = ()=>allReports().find(r=>r.id===active);
  const comparison = ()=>isServerReport(report())?null:getState().reports.find(r=>r.id===$('analytics-gsc').value);
  function render() {
    const reports=allReports(), automatic=getServer().data?.reports||[];
    if(!selectedByUser&&!autoDefaultUsed&&automatic.length){active=automatic[0].id;autoDefaultUsed=true;}
    $('analytics-server-status').textContent=regionalServerStatus(getServer());
    $('analytics-server-status').setAttribute('role',getServer().error?'alert':'status');
    ...
    const matches=isServerReport(r)?[]:getState().reports.filter(s=>s.kind==='pages'&&s.start===r?.start&&s.end===r?.end);
    $('analytics-gsc').disabled=isServerReport(r);
    $('analytics-gsc').replaceChildren(option('',isServerReport(r)?'自動実績は取得元別に表示':r?.origin==='google'?'一括取得した検索実績を使用':'検索実績は未選択'),...matches.map(s=>option(s.id,`${s.searchType} / ${s.filter}`)));
    if(matches.some(s=>s.id===previous))$('analytics-gsc').value=previous;
    $('analytics-scope').textContent=isServerReport(r)?`${serverReportLabel(r)}。${r.source==='ga4'?'PV・ページごとのユーザー数・入口セッション・問い合わせを表示します。検索指標は「自動取得・Search Console」を選ぶと表示されます。':'検索クリック・表示回数・平均順位を表示します。アクセス指標は「自動取得・GA4」を選ぶと表示されます。'} 自動実績の取得元ごとに期間が異なる場合があるため、同じ期間として合成しません。`:r?`${r.start}〜${r.end} / GA4：${r.gaTimezone} / GSC：America/Los_Angeles。取得：${new Date(r.importedAt).toLocaleString('ja-JP')}。${comparison()?`検索条件：${comparison().searchType} / ${comparison().filter}`:'検索条件：ウェブ・全デバイス・全ての国（API取得時）'}`:'実績は未取得です。公開直後や行が返らない地域を0件と表示しません。';
    ...
    $('analytics-export').disabled=!r;$('analytics-delete').disabled=!r||busy||isServerReport(r);
  }
  function disconnect() {token='';expires=0;$('analytics-sync').disabled=true;$('analytics-disconnect').disabled=true;$('analytics-connection-status').textContent='Googleの手動接続は未接続です。サーバーの自動更新には影響せず、自動取得実績と保存済みCSVは閲覧できます。';}
    ...
    try {
      ...
      const client=window.google.accounts.oauth2.initTokenClient({client_id:c.clientId,scope:GOOGLE_SCOPES,
        callback:response=>{
          ...
          $('analytics-connection-status').textContent='Googleの手動接続は接続済みです。任意の期間を指定して「Googleから手動取得」を押してください。自動更新とは別の取得です。';
  ...
  $('analytics-sync').addEventListener('click',async()=>{
    ...
    try {
      ...
      selectedByUser=true;active=incoming.id;await save(putAnalyticsReport(getState(),incoming));render();
  ...
  $('analytics-report').addEventListener('change',()=>{selectedByUser=true;active=$('analytics-report').value;render();});
  $('analytics-gsc').addEventListener('change',render);
  $('analytics-export').addEventListener('click',()=>{
    const r=report();if(!r)return;
    const rows=joinedMetrics(r,comparison(),catalog);
    download(`eltori-access-${r.start}-${r.end}.csv`,csvString([['地域','本番URL','取得方法','開始日','終了日','検索クリック','検索表示回数','平均掲載順位','PV','総ユーザー数','流入セッション','問い合わせ完了数'],...rows.map(row=>[row.page.fullName,`https://layr.co.jp${row.page.path}`,isServerReport(r)?`自動取得・${r.source}`:r.origin,r.start,r.end,...['clicks','impressions','position','views','users','sessions','inquiries'].map(k=>row[k]??'')])]),'text/csv;charset=utf-8');
  });
  $('analytics-delete').addEventListener('click',async()=>{
    if(!active||isServerReport(report())||!confirm('このアクセス実績を削除しますか？必要なら先にJSONバックアップを保存してください。'))return;
  ...
  $('analytics-csv-save').addEventListener('click',async()=>{if(!pending)return;try{selectedByUser=true;active=pending.id;await save(putAnalyticsReport(getState(),pending));clearPreview();render();message('アクセス実績を保存しました。同じ期間の検索実績を上の選択欄で組み合わせられます。');}catch(error){message(error.message,true);}});
