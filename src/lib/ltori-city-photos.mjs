...
export function normalizeCityPhotos(citySlug, record) {
  ...
  return photos.length >= 3 ? {schemaVersion: 1, citySlug, status: 'ready', ...(record.selectionVersion === 'landmarks-v2' ? {selectionVersion: 'landmarks-v2'} : {}), fetchedAt: new Date(fetchedMs).toISOString(), photos} : null;
}

export function getSeedCityPhotos(citySlug) {
  const seed = Object.hasOwn(seeds, citySlug) ? seeds[citySlug] : null;
  return seed?.selectionVersion === 'landmarks-v2' ? normalizeCityPhotos(citySlug, seed) : null;
}
...
export function renderCityPhotos(area, record) {
  ...
  return `<section class="lt-city-photos" id="${prefix}-title" data-city-photos aria-label="${escape(city.fullName)}の観光名所の写真">
    <div class="lt-wrap lt-city-photos-head"><div class="lt-city-photos-controls" data-city-photo-controls hidden><button type="button" data-city-photo-prev aria-label="前の写真を見る" aria-controls="${prefix}-viewport">←</button><button type="button" data-city-photo-next aria-label="次の写真を見る" aria-controls="${prefix}-viewport">→</button><button type="button" data-city-photo-pause aria-pressed="false" aria-controls="${prefix}-viewport">自動送りを止める</button></div></div>
    <div class="lt-city-photos-viewport" id="${prefix}-viewport" data-city-photo-viewport tabindex="0" role="region" aria-label="${escape(city.locality)}の写真。左右にスクロールできます"><ul class="lt-city-photos-track">${data.photos.map(photo => `<li class="lt-city-photo"><img src="${escape(photo.url)}" width="${photo.width}" height="${photo.height}" alt="${escape(photo.alt)}" loading="lazy" decoding="async" referrerpolicy="no-referrer"></li>`).join('')}</ul></div>
